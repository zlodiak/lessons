...
function toggleSpoiler(el,s,h){
	el.className = el.className == 'sp-head folded' ? 'sp-head unfolded' : 'sp-head folded';
	var e=el.parentNode.parentNode.getElementsByTagName('div')[2];
	if (e.style.display=='none'){e.style.display='block';if(h!='')el.innerHTML=h;}
	else{e.style.display = 'none';if(s!='')el.innerHTML = s;}
}

function jf_resize_editorbox(direction)
{
	var inc_value	   = 10;
	var current_height = parseInt( document.postmodify.message.rows );
	var new_height     = 0;
	current_height     = current_height ? current_height : 15;
	if ( current_height >= 5 ) {
		if ( direction == 'up' ) {
		...
		if ( new_height > 4 && new_height < 106) {
			document.postmodify.message.rows = new_height;
		}
	}
}