	...
	if($(link).hasClass('loading')){
		...
		$.post('/json/vote/',{ti:target_id, tt:target_type, pti: post_target_id, v:vote},function(json){	 
			if(json.messages == 'ok'){					
				...
				// обновим класс у коммента
				if( json.sign == 'positive' ){
  				$('#comment_'+target_id+' > .comment_body > .message').removeClass('bad bad1 bad2 bad3 bad4 bad5')
				}else{
  				if( !$('#comment_'+target_id+' > .comment_body > .message').hasClass('bad') ){
    				$('#comment_'+target_id+' > .comment_body > .message').addClass('bad bad1');
  				}
				}
...
/**
 * функция вызывается по нажатию кнопки "удалить" (при редактировании коммента)
 */
function comment_delete(form, button){
    ...
    if (confirm("Точно удаляем?")) {
        ...
        $(form).ajaxSubmit({
            ...
            success: ajaxFormSuccess(function(json, statusText, xhr, jqForm){
                ...
                $('> .comment_body', comment).html('<div class="author_banned">НЛО прилетело и опубликовало эту надпись здесь</div>');
    ...
    {
        $(form).hide();
        $(form).parent().siblings('.reply').show();
...
function comment_show_reply_form(comment_id){
	...
	mention_autocomplete($('#comment_text'))
	return false;
}
...
function comment_show_edit_form(comment_id){
	...
	$.post('/json/comment/', { comment_id: comment_id, action: 'source'}, function(json){
		...
		if(json.messages == 'ok'){
			...
			$('.edit,.delete',form).removeClass('hidden');
...
$(document).ready(function(){
	...
	$('#comments .comment_item .to_parent').live('click',function(){		
			...
			$.scrollTo( $('#comment_'+parent_id) , 800,	{ axis: 'y', offset: -10 } );
	...
	comment_text.live('keyup change blur focus', function (e) {
		if( !empty($(this).val()) ){
			$('input.disableble', comments_form).attr('disabled', false);
		}else{
			$('input.disableble', comments_form).attr('disabled', true);
		}
	});
